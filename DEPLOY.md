...
以下端点已从服务中移除或不再维护，如果客户端仍在调用，建议尽快升级或移除相关调用：

| 端点 | 废弃时间 | 状态 | 说明 |
|------|---------|------|------|
| `POST /api/event_logging/batch` | 2026-01-01 | ❌ 已废弃 | 旧版事件日志批量上报接口，已在启动脚本中静默处理，返回 410 Gone。每小时整点会记录一次 warn 日志用于监控。 |
